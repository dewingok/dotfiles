...
local function trim_trailing_whitespace(bufnr)
  ...
  if trim_trailspace_excluded_filetypes[bo.filetype] then return end

  local ok, trailspace = pcall(require, 'mini.trailspace')
  if not ok then return end

  local view = vim.fn.winsaveview()
  vim.api.nvim_buf_call(bufnr, function() trailspace.trim() end)
...
vim.api.nvim_create_autocmd('BufWritePre', {
  ...
  callback = function(args) trim_trailing_whitespace(args.buf) end,
})
